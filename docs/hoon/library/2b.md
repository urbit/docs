...
gate applied to each element of the original list. `b` is called with a #### tuple the head is an element of `a` and the tail is the state `c`, and should
...
#### Examples
    ...
    > =l (limo ~[7 8 9])                ::joins two lists into a list of pairs
    > %+  spun  (limo ~[4 5 6])
