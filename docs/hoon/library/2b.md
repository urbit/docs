...
Produces the index of the first occurrence of `++list` `nedl` in `++list` `hstk`
...
Produces the indices of all occurrences of `++list` `nedl` in `++list` `hstk` as
a `++list` of atoms.
...
binary gate `b` with an element from `a` and an accumulator, producing the final
...
Left fold: moves left to right across a list `a`, recursively slamming a binary
[gate]() `b` with an element from the [`++list`]() and an accumulator, producing
...
produces a loobean. `++sort` then produces a list of the elements of `a`, sorted
according to `b`.
...
Accepts a `++list` `a` and a gate `b`. Produces a list with the gate applied to
...
Accepts a `++list` `a`, a gate `b`, and some state `c`. Produces a list with the
gate applied to each element of the original list. `b` is called with a tuple --
the head is an element of `a` and the tail is the state `c`, and should produce
a tuple of the transformed element and the (potentially modified) state `c`.
...
`(~(rads og eny) 2)` creates a random number less than `2`, seeding the RNG with
entropy (`eny`). The head of the product is the random number, the tail is the
continuation of the RNG.
...
explicitly as in `++spin`. Produces a list with the gate applied to each element
of the original list. `b` is called with a tuple -- the head is an element of
`a` and the tail is the state `c`, and should produce a tuple of the transformed
element and the (potentially modified) state `c`.
