...
Computes whether the products of `a` and `b` hashed with `mug` are in
ascending numeric order, producing a flag.
...
#### Examples
    ...
    > (lor 9 [1 2])                                     ::  lor computes cells as greater than atoms
    %.y
...
Computes whether the products of `a` and `b` double-hashed with `mug` are in
ascending numeric order, producing a flag.
