...
UTF-8 text. `++trip` and `++crip` convert between cord and `++tape`.
...
A `++tank` is one of three cases: a `%leaf` is simply a string; a `%palm` is XX
need more information; and a `%rose` is a list of `++tank` delimted by the
strings in `p`.
...
The remaining part of a `++date`: day, hour, minute, second and a list of `@ux`
for precision.
...
preceded by `%`. This means a term is always cubical. The empty `@tas` has a
special syntax, `$`.
