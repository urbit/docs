...
children at address `2n` (the head) and address `2n+1` (the tail).
...
(`+3`) of a noun. In other words, `++mas` returns the relative address of `a`
with respect to the head or the tail subtrees, depending on which one `a`
descends from.
