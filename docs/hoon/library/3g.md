...
Used in `++coin`. Convenience methods `++scot` and `++scow` (in Section 2eL)
print dimes as `++cord` and `++tape`, respectively. `++slat`, `++slav`, and
`++slaw` are used to parse atoms of specific odors.
...
Mostly used for stacktraces. A `++pint` is a pair of `++hair`, indicating from
`p` to `q`.
...
`++rule` is an empty parsing rule, but it is used to check that parsing rules
match this with `_`.
