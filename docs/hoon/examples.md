...
Finally, the same code transliterated into runes (and in a more concise "kernel"
style).
...
tail is a new `game`. The `:sip` twig declares the head as a new variable,
...
Lines `51` to `53` define the `is` arm, which is `&` (yes) iff either `a` or `z`
has played `here`.
