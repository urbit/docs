...
is not URL-safe, and every regular atom syntax fits in a URL.
...
a flavorless atom with no aura, so we have to cast it back to an
absolute date `@da`.
...
dojo.  It will keep you from accidentally confusing the two,
