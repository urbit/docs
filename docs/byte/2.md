...
geometry.  But it's not really how we'd like to program.  We'd
...
*limb*.  Limbs can be connected with a `.`, making a *wing*.  
...
"find limb `q`, then find limb `y` inside that."
...
the resemblance isn't exact.  A core has a set of computed
...
As you'd expect with a computed attribute, an expression which
uses a label can't know whether the subject will resolve it to a
surface fragment (a *leg*) or to an arm.  
...
on one line in the dojo, for instance.)

And when we print `b`, or any core, we see that typed noun
printing can't be an invertible function in Hoon. 

We actually can read the battery (with `+2:b` above), but it's
just Nock code, not typed data.  We don't even print the payload
data, though we print the payload type.  The `<2.spy>` just shows
a core with two arms, for which `spy` is a hash code.
...
stack with almost a thousand total arms.

To be exact, the syntax `<283.xzp 42.ajz 407.xtr 110.xht 1.ztu
$151>` denotes a system of six cores, each of whose payload is
another core, until the innermost payload `%151` (the Hoon
version).  This stack is our standard library, from which comes
`add` and other fine functions.  (We don't know how to define a
Hoon function yet, but we're getting close.)
